...
class RunRedmapper_haloPixelTask(object):
    """
    Class to run redmapper on a single healpix pixel, for distributed runs.
    """
    ...
    def run(self):
        ...
        halo = RunHalo(self.config)

        if not os.path.isfile(halo.filename):
            halo.run()
            halo.output(savemembers=True, withversion=False)
        else:
            self.config.logger.info("halo run file %s already present.  Skipping..." % (halo.filename))
