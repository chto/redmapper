...
class RunHalo(ClusterRunner):
    ...
    def _additional_initialization(self, **kwargs):
        ...
        """
        self.runmode = 'percolation' #Using percolation ro and beta 
    ...
    def run(self, *args, **kwargs):
        ...
        return super(RunHalo, self).run(*args, **kwargs)
    ...
    def _more_setup(self, *args, **kwargs):
        ...
        self.config.logger.info("%d: runcat using catfile: %s" % (self.config.d.hpix, self.config.catfile))
        ...
        self.keepz = kwargs.pop('keepz', True)
        ...
        if self.keepid:
            ...
            # Reverse sort by mass
            st = np.argsort(self.cat.m200[use])[::-1]
        ...
        self.do_percolation_masking = True #get pfree
        self.do_lam_plusminus = True #dlambdadz
        self.record_members = True #
        self.do_correct_zlambda = True
        self.do_pz = True #Obtain P(z)
        ...
        self.maxiter = 5
        self.tol = 1E-5 #zlambda convergence 
        self.maskfraccut = 0.7 #maskfrac_cut
    ...
    def _process_cluster(self, cluster):
        ...
        #print(reduce(getattr, self.config.centerclass.split('.'), sys.modules[__name__]))
...
#
        ...
        for i in xrange(self.maxiter):
            ...
            if (((cluster.Lambda/cluster.scaleval) < self.config.percolation_minlambda)):
                bad = True
                continue
            ...
            if cluster.z_lambda < 0.0:
                bad = True
                continue
            if np.abs(z_use-cluster.z_lambda)< self.tol:
                break
            else:
              z_use = cluster.z_lambda
        #end for 
        if bad:
        ...
        cluster.ncent_good = 1

        #print("{0:.3E}".format(cluster.m200),z_use, cluster.lambda_c, cluster.z_lambda)
        #assert False
